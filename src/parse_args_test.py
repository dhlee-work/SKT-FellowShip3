...
parser.add_argument('--no_join_condition', action='store_true', default=False,
...
parser.add_argument('--demo_db', type=str, default='d_cem',
...
parser.add_argument('--use_graph_encoding', action='store_true', default=False,
                    help='If set, use graph encoding as input.')
parser.add_argument('--use_picklist', action='store_true', default=False,
...
parser.add_argument('--top_k_picklist_matches', type=int, default=2,
...
parser.add_argument('--no_anchor_text', action='store_true',default=False,
                    help='If add, add only the special value token but not the value text to the schema representation')
parser.add_argument('--question_split', action='store_true',default=True,
                    help='split dataset based on questions (default: False)')
parser.add_argument('--query_split', action='store_true',default=False,
...
parser.add_argument('--pretrained_transformer', type=str, default='bert-base-multilingual-uncased', # 'bert-base-uncased' # 'bert-base-multilingual-cased'
                    help='Specify pretrained transformer model to use.')
parser.add_argument('--fix_pretrained_transformer_parameters', action='store_true', default=False,
                    help='If set, no finetuning is performed on the pretrained BERT embeddings (default: False).')
parser.add_argument('--use_typed_field_markers', action='store_true', default=False,
...
parser.add_argument('--text_vocab_min_freq', type=int, default=0,
                    help='Minimum word frequency in natural language vocabulary (default: 1)')
parser.add_argument('--program_vocab_min_freq', type=int, default=0,
...
parser.add_argument('--enumerate_ground_truth', action='store_true',
                    help='Sample ground truth queries during training when there are multiple correct ones '
                         '(default: False)')
parser.add_argument('--save_nn_weights_for_visualizations', action='store_true',default=False,
...
parser.add_argument('--denormalize_sql', action='store_true',default=True,
                    help='Denormalize SQL queries (default: False)')
parser.add_argument('--omit_from_clause', action='store_true',default=False,
                    help='Remove FROM clause from SQL query in data preprocessing (default: False)')
parser.add_argument('--use_oracle_tables', action='store_true', default=False,
...
parser.add_argument('--random_field_order', action='store_true', default=False,
                    help='If set, use random field order in augmented table schema (default: False).')
parser.add_argument('--augment_with_wikisql', action='store_true', default=False,
                    help='If set, augment training data with WikiSQL (default: False)')
parser.add_argument('--process_sql_in_execution_order', action='store_true',default=True,
                    help='Generate and process SQL clauses in execution order (default: False)')
parser.add_argument('--sql_consistency_check', action='store_true', default=True,
...
parser.add_argument('--decoder_input_dim', type=int, default=400,
...
parser.add_argument('--res_input_dropout_rate', type=float, default=0.2,
...
parser.add_argument('--max_in_seq_len', type=int, default=512,
                    help='Maximum input length (default: 120)')
parser.add_argument('--max_out_seq_len', type=int, default=60,
                    help='Maximum output length (default: 120)')
parser.add_argument('--use_lstm_encoder', action='store_true', default=True,
                    help='If set, use LSTM encoder on top of pretrained transformer encoders (default: False)')
parser.add_argument('--use_meta_data_encoding', action='store_true', default=True,
                    help='If set, encode meta data features in the DB schema (default: False)')

# RNNs
parser.add_argument('--encoder_hidden_dim', type=int, default=400,
...
parser.add_argument('--use_additive_features', action='store_true', default=False,
...
parser.add_argument('--ff_input_dropout_rate', type=float, default=0.4,
...
parser.add_argument('--num_epochs', type=int, default=200, # 200
                    help='maximum number of pass over the entire training set (deult: 20)')
parser.add_argument('--num_wait_epochs', type=int, default=200, # 200
...
parser.add_argument('--num_steps', type=int, default=20000, # 20000
...
parser.add_argument('--num_peek_steps', type=int, default=100, # 400
...
parser.add_argument('--dev_batch_size', type=int, default=12,
...
parser.add_argument('--bert_finetune_rate', type=float, default=0.00006,
                    help='BERT finetune rate (default: 0)')
parser.add_argument('--learning_rate', type=float, default=0.0005,
                    help='learning rate (default: 0.0001)')
parser.add_argument('--learning_rate_scheduler', type=str, default="inverse-square",
                    help='learning rate scheduler (default: step)')
parser.add_argument('--trans_learning_rate_scheduler', type=str, default="inverse-square",
...
parser.add_argument('--num_warmup_steps', type=int, default=4000, # 4000
...
parser.add_argument('--bs_alpha', type=float, default=1.05,
